...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  return { title: `${dict.account.forgotPassword} | Petiwell` };
}

export default async function ForgotPage(props: Props) {
  const params = await props.params;
  const locale = (isLocale(params.locale) ? params.locale : "tr") as Locale;
  const dict = getDictionary(locale);
  ...
  return (
    <section className="section-shell">
      <div className="mx-auto max-w-md rounded-2xl border border-line bg-surface p-6 shadow-soft">
        <h1 className="section-title">{dict.account.forgotPassword}</h1>
