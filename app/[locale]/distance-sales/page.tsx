...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  const doc = getConsumerPolicy(locale, "distanceSales");
  return { title: `${doc.title} | Petiwell` };
}

export default async function DistanceSalesPage(props: Props) {
  const params = await props.params;
  const locale = (isLocale(params.locale) ? params.locale : "tr") as Locale;
  return <PolicyDocument doc={getConsumerPolicy(locale, "distanceSales")} />;
}
